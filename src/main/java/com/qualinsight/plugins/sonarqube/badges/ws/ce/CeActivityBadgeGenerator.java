...
/**
 * Generates SVG badge based on a Compute Engine Activity status. A reusable {@link InputStream} is kept in a cache for each generated image
...
public class CeActivityBadgeGenerator {
  ...
  private final Map<SVGImageTemplate, Map<CeActivityBadge, InputStream>> ceActivityBlinkingBadgesMap = new EnumMap<>(
      SVGImageTemplate.class);
   ...
   *          {@link SVGImageGenerator} service to be used.
   * @param fontReplacer
   *          {@link SVGImageMinimizer} service to be used.
   */
  ...
  /**
   * Returns an {@link InputStream} holding the content of the generated image for the provided quality gate status. All {@link InputStream}
   * s are cached for future reuse.
   *
   * @param status
   *          quality gate status for which the image has to be generated
   * @param template
   *          {@link SVGImageTemplate} to be used
   * @param blinkingValueBackgroundColor
   *          true if the badge must be blinking in case of quality gate error
   * @return {@link InputStream} holding the expected SVG image
   * @throws IOException
   *           if a IO problem occurs during streams manipulation
   */
  public InputStream svgImageInputStreamFor(final CeActivityBadge status, final SVGImageTemplate template,
      final boolean blinkingValueBackgroundColor)
    throws IOException {
    ...
    if (workingMap.containsKey(status)) {
      ...
      svgImageTransformedInputStream = workingMap.get(template).get(status);
      ...
      final SVGImageData data = SVGImageData.Builder.instance(this.imageGenerator.fontProvider()).withTemplate(template)
          .withLabelText(LABEL_TEXT).withLabelBackgroundColor(SVGImageColor.DARK_GRAY).withValueText(status.displayText())
          .withValueBackgroundColor(status.displayBackgroundColor()).build();
      ...
      final Map<String, Object> parameters = ImmutableMap.<String, Object> builder()
          .put("IS_BLINKING_BADGE", Boolean.toString(blinkingValueBackgroundColor && status.equals(CeActivityBadge.FAILED))).build();
      // minimze SVG stream
      ...
      // put it into cache
      workingMap.get(template).put(status, svgImageTransformedInputStream);
    }
